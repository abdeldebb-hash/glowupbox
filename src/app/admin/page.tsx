...
async function getStats() {
  try {
    const [boxes, products, articles, options] = await Promise.all([
      prisma.box.count({ where: { active: true } }),
      prisma.product.count({ where: { active: true } }),
      prisma.article.count({ where: { published: true } }),
      prisma.option.count(),
    ])
    return { boxes, products, articles, options }
  } catch {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {SECTIONS.map(s => {
            ...
            return (
              <Link key={s.href} href={s.href}
                className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md hover:border-[rgba(233,30,140,.2)] hover:bg-[rgba(233,30,140,.01)] transition-all duration-200 flex items-start gap-4 group">
