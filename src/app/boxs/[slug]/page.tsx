...
const TESTIMONIALS = [
  { text: '"J\'ai reçu ma box et le suivi WhatsApp a tout changé. Résultats visibles en 3 semaines !"', name: 'Sara M.',  meta: 'Casablanca' },
...
export default async function BoxDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const box     = rows[0]
  const related = await tursoQuery('SELECT * FROM Box WHERE active=1 AND slug!=? ORDER BY "order" ASC LIMIT 3', [slug])
  const products: string[] = (() => { try { return JSON.parse(String(box.products)) } catch { return [] } })()
      ...
      <section className="bg-dark-gradient pt-40 pb-20 px-5 relative overflow-hidden text-center">
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          <p className="text-white/40 text-[11px] uppercase tracking-widest mb-5 flex items-center justify-center gap-2">
            <a href="/" className="hover:text-coral transition-colors">Accueil</a><span>›</span>
            <a href="/boxs" className="hover:text-coral transition-colors">Boxs</a><span>›</span>
            <span className="text-white/65">{String(box.name)}</span>
          </p>
          <span className="inline-flex items-center bg-[rgba(233,30,140,.15)] border border-[rgba(233,30,140,.28)] text-coral px-5 py-2 rounded-full text-[10px] font-semibold uppercase tracking-widest mb-6">
            {String(box.tag || box.skinLabel)}
          </span>
          <h1 className="font-playfair font-bold italic text-white mb-5" style={{ fontSize:'clamp(2.8rem,6vw,5.5rem)' }}>
            <span className="gradient-text">{String(box.name)}</span>
          </h1>
          <p className="text-white/55 text-base max-w-lg mx-auto leading-relaxed">{String(box.accroche || box.description)}</p>
        </div>
      </section>
      ...
      <section className="py-20 px-5 bg-white">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[5fr_7fr] gap-12 items-start">
          <div className="rounded-3xl overflow-hidden aspect-[3/4] shadow-[0_24px_72px_rgba(26,26,46,.1)] relative group">
            <span className="absolute top-5 left-5 z-10 bg-brand-gradient text-white text-[9px] font-bold uppercase tracking-widest px-4 py-2 rounded-full">{String(box.skinLabel)}</span>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={box.image ? String(box.image) : '/images/peau-sensible.jpg'} alt={String(box.name)} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
          </div>
          <div className="rounded-3xl overflow-hidden border border-[rgba(233,30,140,.08)] shadow-[0_12px_40px_rgba(26,26,46,.06)]">
            <div className="px-8 py-5 bg-brand-black flex items-center justify-between">
              <h2 className="font-playfair font-bold italic text-white text-2xl">{String(box.name)}</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2">
              <div className="p-8 bg-white border-r border-[rgba(233,30,140,.07)]">
                ...
                <p className="font-playfair font-bold italic text-brand-black text-xl leading-snug mb-3">{String(box.accroche || String(box.description).split('.')[0])}</p>
                <p className="text-brand-gray text-[13px] leading-relaxed mb-5">{String(box.description)}</p>
                ...
                <ul className="space-y-2">
                  {['Accessoires offerts','Fiche routine personnalisée','Suivi WhatsApp 1 mois','Livraison Maroc'].map(item => (
                    <li key={item} className="flex items-center gap-2 text-[13px] text-brand-gray">
                      <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background:'linear-gradient(135deg,#E91E8C,#FFB347)' }} />{item}
                    </li>
                  ))}
                </ul>
              </div>
              <div className="p-8 bg-soft-pink">
                ...
                <div className="space-y-2.5">
                  <GradientButton href={waUrl(String(box.waMessage))} size="sm" external className="w-full justify-center">
              ...
              <ul className="space-y-2">
                {col.items.map(item => (
                  <li key={item} className="flex items-start gap-2 text-white/65 text-sm">
                    <span className="w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0" style={{ background:'linear-gradient(135deg,#E91E8C,#FFB347)' }} />{item}
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="max-w-5xl mx-auto">
          <ScrollReveal>
            ...
            <h2 className="font-playfair font-bold text-brand-black mt-2" style={{ fontSize:'clamp(2rem,4vw,3.5rem)' }}>
              Ce que contient ta <span className="gradient-text">{String(box.name)}</span>
            </h2>
          </ScrollReveal>
            ...
            {SUIVIS.map(({ icon: Icon, title, desc }, i) => (
              <ScrollReveal key={title} delay={i * 0.1}>
                <div className="bg-white rounded-3xl p-8 border border-[rgba(233,30,140,.08)] hover:-translate-y-1.5 hover:shadow-[0_24px_48px_rgba(233,30,140,.1)] transition-all duration-300">
                  <div className="w-12 h-12 rounded-2xl bg-[rgba(233,30,140,.08)] flex items-center justify-center mb-5"><Icon className="w-5 h-5 text-fuchsia" /></div>
            ...
            {TESTIMONIALS.map((t, i) => (
              <ScrollReveal key={i} delay={i * 0.1}>
                <div className="bg-soft-pink rounded-3xl p-7 border border-[rgba(233,30,140,.08)] hover:-translate-y-1 transition-transform duration-300">
                  <div className="flex gap-0.5 mb-4">{[...Array(5)].map((_,j) => <Star key={j} className="w-3.5 h-3.5 fill-[#FFB347] text-[#FFB347]" />)}</div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div><p className="font-semibold text-brand-black text-sm">{t.name}</p><p className="text-brand-gray text-xs mt-0.5">{t.meta}</p></div>
                  </div>
                </div>
              </ScrollReveal>
            ))}
      ...
      {related.length > 0 && (
        <section className="bg-soft-pink py-20 px-5">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              {related.map((b, i) => (
                <ScrollReveal key={String(b.slug)} delay={i * 0.1}>
                  <motion.a href={`/boxs/${String(b.slug)}`} whileHover={{ y:-6 }}
                    ...
                    <div className="h-36 relative overflow-hidden">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={b.image ? String(b.image) : '/images/peau-sensible.jpg'} alt={String(b.name)} className="w-full h-full object-cover" />
                    </div>
                    <div className="p-5">
                      <p className="text-fuchsia text-[9px] font-bold uppercase tracking-widest mb-1.5">{String(b.skinLabel)}</p>
                      <p className="font-playfair font-bold italic text-brand-black text-lg mb-3">{String(b.name)}</p>
                      <span className="flex items-center gap-1 text-fuchsia text-[11px] font-bold uppercase tracking-wide">Voir la box <ArrowRight className="w-3 h-3" /></span>
                    </div>
                  </motion.a>
                </ScrollReveal>
              ))}
      ...
      <section className="bg-brand-gradient py-20 px-5 text-center relative overflow-hidden">
        ...
        <ScrollReveal className="relative z-10">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            <GradientButton href={waUrl(String(box.waMessage))} variant="white" size="lg" external>
              <MessageCircle className="w-4 h-4" />Je veux cette box
            </GradientButton>
